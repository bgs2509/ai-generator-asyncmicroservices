...
- **Submodule**: Added as `.ai-framework/` submodule to your project (use paths like `.ai-framework/docs/`)
...
### Framework Directory (`.ai-framework/`)
...
- **`.env.example`**: Environment configuration template with all variables
  - **Framework template**: `.ai-framework/templates/infrastructure/.env.example` (203 lines with all variables)
...
3. **Follow framework patterns** from `docs/` *(or `.ai-framework/docs/` when used as submodule)*
4. **Never modify** `.ai-framework/` content
